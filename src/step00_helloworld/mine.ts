...
//7-Bigint
let largeNumber: bigint = 9007199254740991n; // Large integer number value. Stores numbers larger than 2^53

//8-Any
// Used when you don't want a particular variable to have a specific type.
let randomValue: any = 23;
randomValue = "Ramzan";
randomValue = 1990;
randomValue = true;
...
let person: [string, number] = ["Ramzan", 23];
...
function addItemToCart(item: unknown, price: number): number {
...
function calculateTotal(cart: { item: unknown; price: number }[]): number {
  let total = 0;
